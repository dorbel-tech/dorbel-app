...
function* post() {
  ...
  logger.debug({event_id: eventId, registered_user_id: userId}, 'Registering to an open house event...');
  const result = yield service.register(eventId, userId);
  logger.info({event_id: eventId, registered_user_id: userId}, 'Registration created');
